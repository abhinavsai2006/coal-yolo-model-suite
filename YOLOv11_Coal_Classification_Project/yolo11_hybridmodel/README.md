# Hybrid EfficientNet-B3 + Attention Model – Coal Classification (YOLOv11 Project)
**High-performance coal classification system using a hybrid EfficientNet-B3 (replacing earlier ResNet50) backbone with custom attention and fusion modules.**
## 📊 Final Model Performance
| Metric | Value |
| **Test Accuracy (Single)** | **91.21%** |
| **Test Accuracy (HFlip TTA)** | **91.76%** |
| **Validation Accuracy (best epoch)** | 93.33% |
| **Model Size** | ~98 MB |
| **Inference Speed (RTX 3050)** | 55–105 ms/image |
### Current Confusion & Class Metrics
See `evaluation_b3_single/` and `evaluation_b3_hflip_tta/` for confusion matrices and per-class accuracy plots generated from the final checkpoint (`runs/b3_seed1337/weights/best.pt`).
### Inference (Prediction)
import torch, sys
from pathlib import Path
from PIL import Image
import torchvision.transforms as transforms
from models.hybrid_model_pretrained import EfficientNetHybridWrapper
MODEL_PATH = "runs/b3_seed1337/weights/best.pt"  # Final checkpoint
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = EfficientNetHybridWrapper(num_classes=6)
...
CLASSES = ['destructive_coal','fully_pulverized_coal','non_destructive_coal','not_coal','pulverized_coal','strongly_destructive_coal']
print(f"Predicted: {CLASSES[pred.item()]} (Confidence: {conf.item():.2%})")
python predict_hybrid.py --model-path runs/b3_seed1337/weights/best.pt --tta hflip
python evaluate_hybrid.py --model-path runs/b3_seed1337/weights/best.pt
Optional TTA:
python evaluate_hybrid.py --model-path runs/b3_seed1337/weights/best.pt --tta hflip
Expected: ~91.21% (single) / ~91.76% (hflip TTA)
├── 📊 evaluation_b3_single/            # Single model evaluation artifacts
├── 📊 evaluation_b3_hflip_tta/         # HFlip TTA evaluation artifacts
├── 🏆 runs/
│   └── b3_seed1337/                    # Final EfficientNet-B3 hybrid run
│       ├── weights/
│       │   └── best.pt                 # Final checkpoint ⭐
│       ├── train_log.txt (if retained)
│       └── results.png (if retained)
## 🏗️ Model Architecture (Updated)
Pretrained EfficientNet-B3 Backbone (ImageNet weights)
1. **Pretrained Backbone**: EfficientNet-B3 with ImageNet weights
- **Total Parameters**: ~29M
- **Trainable Parameters**: ~29M
### Historical vs Final Results
| Model | Backbone | Val Acc (best) | Test Acc | Notes |
| Early Model | ResNet50 | 93.85% | 90.11% | Previous best single |
| Final Single | EfficientNet-B3 | 93.33% | 91.21% | Improved generalization |
| Final + TTA | EfficientNet-B3 | 93.33% | 91.76% | HFlip TTA only |
Key Finding: EfficientNet-B3 single model with simple horizontal flip TTA slightly surpasses prior ResNet50; complex ensembles & heavy TTA gave diminishing returns.
### Python Dependencies (Core)
## 📈 Performance Benchmarks (Updated)
## 📚 Dataset Information
└── test/          # 364 images (balanced formats)
### Class Distribution & Balance
Relatively uniform across 6 classes; minor variance (<10%)—no rebalancing required. Stratified split ensured representative validation & test sets.
## 🔄 Optional Next Steps Toward ≥92%
If pushing beyond 91.76%:
1. Temperature scaling or calibration-aware fine-tune (2–3 epochs @ LR 2e-5).
2. Selective last 2 EfficientNet stages + attention modules fine-tune with lower dropout.
3. Light MixUp (alpha=0.2) + CutMix disabled; monitor validation lift.
4. Higher resolution (256→224 center crop) only if no overfitting signs.
5. Deploy ONNX + TensorRT for speed; confirm no accuracy regression.
## ✅ Summary
Final deployed artifact: `runs/b3_seed1337/weights/best.pt`.
Evaluations: `evaluation_b3_single/` (91.21%), `evaluation_b3_hflip_tta/` (91.76%).
Use `predict_hybrid.py` with `--tta hflip` for best result.
